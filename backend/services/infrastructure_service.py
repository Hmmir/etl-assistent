...
"""
    ...
    def deploy_dag_to_airflow(self, dag_content: str, dag_name: str) -> Dict:
        """Развертывание DAG в Airflow с автоматической активацией"""
        try:
            ...
            # Автоматически активируем DAG через API
            self._activate_dag_automatically(dag_name.replace('.py', ''))
            
            return {
                "success": True,
                "message": f"✅ DAG {dag_name} развернут и активирован в Airflow",
                "dag_file": str(dag_file_path),
                "airflow_ui": "http://localhost:8081",
                "note": "DAG автоматически активирован и готов к выполнению"
            }
            
        except Exception as e:
            return {"success": False, "error": str(e)}
    
    def _activate_dag_automatically(self, dag_id: str) -> bool:
        """Автоматическая активация DAG через Airflow API"""
        import requests
        import time
        ...
        # Ждем пока DAG загрузится (до 30 секунд)
        for attempt in range(12):  # 12 попыток по 2.5 сек = 30 сек
            try:
                # Проверяем, что DAG загружен
                check = requests.get(f"{airflow_url}/api/v1/dags/{dag_id}", auth=auth, timeout=5)
                if check.status_code == 200:
                    dag_info = check.json()
                    
                    # Активируем DAG если он приостановлен
